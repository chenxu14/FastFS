...
static void replayNextExtent(struct spdk_bdev_io *bdev_io, bool success, void *cb_arg);
...
static void parseExtent(ByteBuffer *buffer, uint32_t startOff, uint32_t skipOps) {
  ...
  if (flag == 1 && journal->nextExtentId > 0) {
    buffer->clear();
    journal->offset = static_cast<uint64_t>(journal->nextExtentId) << fs_context.extentBits;
    journal->curBlock = 0;
  ...
  if (blockCorect) {
    // reset tail block
    journal->tail_block->putBytes(0, buffer->p_buffer_ + offset, fs_context.blockSize);
    ...
    SPDK_WARNLOG("block %d's data not as expected, FastFS does't stop gracefully?\n",
                 journal->curBlock >> fs_context.blockBits);
  }
  fastfs->freeBuffer(buffer);
  SPDK_NOTICELOG("replay journal finished, total replay %ld OPs\n", journal->replayOps);
  fastfs->dumpInfo();
...
static void replayFirstExtent(struct spdk_bdev_io *bdev_io, bool success, void *cb_arg) {
  if (bdev_io) {
    spdk_bdev_free_io(bdev_io);
  }
  ByteBuffer *buffer = reinterpret_cast<ByteBuffer *>(cb_arg);
...
static void journalWriteComplete(struct spdk_bdev_io *bdev_io, bool success, void *cb_arg) {
...
int FastJournal::pollEditOp() {
  ...
  while (editOp->phrase ^ phrase) {
    ...
    if (tail_block->writable(size + 5 /*opcode(1) and size(4)*/)) {
      ...
      int rc = spdk_bdev_write(fs_context.bdev_desc,
                               fs_context.bdev_io_channel,
                               tail_block_buffer,
                               offset,
                               fs_context.blockSize,
                               journalWriteComplete,
                               this);
      if (rc == -ENOMEM) {
...
void FastJournal::initObjPool() {
  ...
  limitIdx = poolSize - 1;
  poolMask = limitIdx;
  editObjs = new std::vector<EditOp, MemAllocator<EditOp>>(MemAllocator<EditOp>(fs_context.localNuma, 64));
...
void FastJournal::dumpInfo() {
  ...
  SPDK_NOTICELOG("epoch %d, txid %ld, extentId %ld, nextExtent %s, blockId %d, num_ops %d\n",
                 epoch,
                 txid,
                 extentId,
                 nextExtent.c_str(),
