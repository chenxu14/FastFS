...
static void parse_options(int argc, char *argv[]) {
  static struct option options_config[] = {
      {"spdk_conf",      required_argument, 0,                'c'},
...
static void bdev_init_done(int rc, void *cb_arg) {
  ...
  rc = spdk_bdev_open_ext(fs_context.bdev_name, true, fuse_event_cb, NULL,
...
static void bdev_init_start(void *arg) {
  g_json_data = spdk_posix_file_load_from_name(
      options.spdk_conf, &g_config_file_size);
  spdk_subsystem_load_config(
...
static FastFS* mount_fastfs() {
  ...
  fuseCtx = reinterpret_cast<struct fastfs_fuse_context*>(
      spdk_thread_get_ctx(thread));
...
int main(int argc, char **argv) {
  ...
  struct fuse_args args =
      FUSE_ARGS_INIT((int)fuseArgsPtr.size(), fuseArgsPtr.data());
