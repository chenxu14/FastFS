...
static void fastfs_getattr(fuse_req_t req, fuse_ino_t ino, struct fuse_file_info *fi) {
...
static void fastfs_setattr(fuse_req_t req, fuse_ino_t ino, struct stat *attr, int to_set, struct fuse_file_info *fi) {
  // TODO chenxu14 support chmod, time and truncate
  fastfs_getattr(req, ino, fi);
}

static void fastfs_lookup(fuse_req_t req, fuse_ino_t parentId, const char *name) {
...
static void create_dir_complete(void *cb_args, int code) {
  ...
  CreateContext *createCtx = reinterpret_cast<CreateContext *>(fuseOp->opCtx->private_data);
  if (code == 0) {
    struct fuse_entry_param e;
    memset(&e, 0, sizeof(e));
...
static void fastfs_mkdir(fuse_req_t req, fuse_ino_t pid, const char *name, mode_t mode) {
  inflights++;
...
static void create_file_complete(void *cb_args, int code) {
  ...
  if (code == 0) {
    ...
    e.attr.st_blksize = FastFS::fs_context.extentSize;
    e.attr.st_blocks = targetInode.extents_->size();
    fuseOp->file->fh = fuseCtx->fastfs->open(createCtx->ino, fuseOp->file->flags | O_SYNC);
...
static void fastfs_create(fuse_req_t req, fuse_ino_t pid, const char *name, mode_t mode, struct fuse_file_info *fi) {
  inflights++;
  FuseOp *fuseOp = allocFuseOp();
  fuseOp->req = req;
  fuseOp->file = fi;
...
static void fastfs_rmdir(fuse_req_t req, fuse_ino_t pid, const char *name) { fastfs_unlink(req, pid, name); }
...
static void fastfs_rename(
  fuse_req_t req, fuse_ino_t olddir, const char *oldname, fuse_ino_t newdir, const char *newname, unsigned int flags) {
  inflights++;
  FuseOp *fuseOp = allocFuseOp();
  fuseOp->req = req;
  fuseOp->opCtx = fuseCtx->fastfs->allocFsOp();
...
static void fastfs_forget(fuse_req_t req, fuse_ino_t ino, uint64_t nlookup) { fuse_reply_none(req); }

static void fastfs_forgetmulti(fuse_req_t req, size_t count, struct fuse_forget_data *forgets) { fuse_reply_none(req); }

static void fastfs_flush(fuse_req_t req, fuse_ino_t fino, struct fuse_file_info *fi) { fuse_reply_err(req, 0); }
...
static void fastfs_fsync(fuse_req_t req, fuse_ino_t fino, int datasync, struct fuse_file_info *fi) {
  inflights++;
  FuseOp *fuseOp = allocFuseOp();
  fuseOp->req = req;
  fuseOp->opCtx = fuseCtx->fastfs->allocFsOp();
...
static void fastfs_open(fuse_req_t req, fuse_ino_t ino, struct fuse_file_info *fi) {
  if (fi->flags & O_TRUNC) {
    inflights++;
    FuseOp *fuseOp = allocFuseOp();
    fuseOp->req = req;
    ...
    fuseOp->ino = ino;
    fuseOp->opCtx = fuseCtx->fastfs->allocFsOp();
    TruncateContext *truncateCtx = new (fuseOp->opCtx->private_data) TruncateContext();
...
static void fastfs_release(fuse_req_t req, fuse_ino_t ino, struct fuse_file_info *fi) {
  uint32_t fd = static_cast<uint32_t>(fi->fh);
  int rc = fuseCtx->fastfs->close(fd);
  fuse_reply_err(req, rc < 0 ? ENOENT : 0);
}

static void fastfs_add_dentry(fuse_req_t req, ByteBuffer *buffer, const char *name, fuse_ino_t ino) {
  size_t len = fuse_add_direntry(req, NULL, 0, name, NULL, 0);
  struct stat stbuf;
  memset(&stbuf, 0, sizeof(stbuf));
  stbuf.st_ino = ino;
  fuse_add_direntry(req, buffer->getBuffer(), buffer->remaining(), name, &stbuf, buffer->position_ + len);
  buffer->skip(len);
}

// TODO chenxu14 consider offset
static void fastfs_readdir(fuse_req_t req, fuse_ino_t ino, size_t size, off_t off, struct fuse_file_info *fi) {
  FastInode &targetInode = (*fuseCtx->fastfs->inodes)[ino - 1];
...
static void fastfs_releasedir(fuse_req_t req, fuse_ino_t fino, struct fuse_file_info *fi) { fuse_reply_err(req, 0); }

static void fastfs_opendir(fuse_req_t req, fuse_ino_t ino, struct fuse_file_info *fi) { fuse_reply_open(req, fi); }

static void read_complete(void *cb_args, int code) {
  FuseOp *fuseOp = reinterpret_cast<FuseOp *>(cb_args);
  ReadContext *readCtx = reinterpret_cast<ReadContext *>(fuseOp->opCtx->private_data);
  if (code == -2) { // EOF
...
static void fastfs_read(fuse_req_t req, fuse_ino_t ino, size_t size, off_t off, struct fuse_file_info *fi) {
  inflights++;
  FuseOp *fuseOp = allocFuseOp();
...
static void write_complete(void *cb_args, int code) {
  FuseOp *fuseOp = reinterpret_cast<FuseOp *>(cb_args);
  WriteContext *writeCtx = reinterpret_cast<WriteContext *>(fuseOp->opCtx->private_data);
  if (code != 0) {
    fuse_reply_err(fuseOp->req, EIO);
  } else {
...
static void
fastfs_write(fuse_req_t req, fuse_ino_t ino, struct fuse_bufvec *buf, off_t off, struct fuse_file_info *fi) {
  inflights++;
...
static void fastfs_getxattr(fuse_req_t req, fuse_ino_t ino, const char *name, size_t size) {
  fuse_reply_err(req, ENOTSUP);
}

static void
fastfs_setxattr(fuse_req_t req, fuse_ino_t ino, const char *name, const char *value, size_t size, int flags) {
  fuse_reply_err(req, ENOTSUP);
}

static void fastfs_removexattr(fuse_req_t req, fuse_ino_t ino, const char *name) { fuse_reply_err(req, ENOTSUP); }
...
static void fastfs_ioctl(fuse_req_t req,
                         fuse_ino_t fino,
                         int cmd,
                         void *arg,
...
static void init_fuse_ops(struct fuse_lowlevel_ops &ops) {
  ...
  ops.rename = fastfs_rename, ops.create = fastfs_create;
